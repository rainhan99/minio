...
5. 为内嵌设计系统依赖 `mds` 所做的改动：`web-app/package.json` 的 `"mds"` 改为 `file:../mds`；
   `console/.gitignore` 新增 `!mds/dist/`（该文件的 `dist/` 规则会忽略 `console/mds/dist/` 全部 485 个
...
`web-app` 的设计系统依赖 `mds` 的上游仓库 `github.com/minio/mds` 已不存在。该依赖现已按锁定提交
`400914d72cb3ffa27d600e0ae1f17ece2182ec22` 内嵌到 `console/mds/`，由 `web-app/package.json` 以
`"mds": "file:../mds"` 引用。**来源、验证依据、协议选型理由、本地改动与可复现性结论全部记录在
`console/mds/UPSTREAM-SOURCE.md`，接入或替换该目录前必须先读该文件。**
...
实测通过（Node v22.20.0、Yarn 4.9.4），且同一输入连续两次构建的 112 个产物文件 SHA-256 逐字节一致。
`web-app/build/` 现为本地工具链产出的基线，与上游 CI 原产物不同（差异来自 `luxon` 去重与工具链版本差异，
非源码改动）。
